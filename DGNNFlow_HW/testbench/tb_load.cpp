...
// Define the arrays
WT_TYPE graphmet_embed_charge_weight_fixed[3][EMB_DIM / 4];
WT_TYPE graphmet_embed_pdgid_weight_fixed[7][EMB_DIM / 4];
WT_TYPE graphmet_embed_continuous_0_weight_fixed[EMB_DIM / 2][NUM_CONT_FEAT];
WT_TYPE graphmet_embed_continuous_0_bias_fixed[EMB_DIM / 2];
WT_TYPE graphmet_embed_categorical_0_weight_fixed[EMB_DIM / 2][EMB_DIM / 2];
WT_TYPE graphmet_embed_categorical_0_bias_fixed[EMB_DIM / 2];
WT_TYPE graphmet_encode_all_weight_fixed[EMB_DIM][EMB_DIM];
WT_TYPE graphmet_encode_all_bias_fixed[EMB_DIM];
WT_TYPE graphmet_bn_all_weight_fixed[EMB_DIM];
WT_TYPE graphmet_bn_all_bias_fixed[EMB_DIM];
WT_TYPE graphmet_bn_all_running_mean_fixed[EMB_DIM];
WT_TYPE graphmet_bn_all_running_var_fixed[EMB_DIM];

// Define the arrays for EdgeConv Linear Layers
WT_TYPE graphmet_edge_conv_linear_weight_fixed[NUM_LAYERS][EMB_DIM][2 * EMB_DIM];

// Define the arrays for EdgeConv Bias Layers
WT_TYPE graphmet_edge_conv_linear_bias_fixed[NUM_LAYERS][EMB_DIM];

// Define the arrays for EdgeConv BatchNorm1D Layers
WT_TYPE graphmet_edge_conv_batchnorm_weight_fixed[NUM_LAYERS][EMB_DIM];
WT_TYPE graphmet_edge_conv_batchnorm_bias_fixed[NUM_LAYERS][EMB_DIM];
WT_TYPE graphmet_edge_conv_batchnorm_mean_fixed[NUM_LAYERS][EMB_DIM];
WT_TYPE graphmet_edge_conv_batchnorm_variance_fixed[NUM_LAYERS][EMB_DIM];

// Define the arrays for final forward layer
WT_TYPE graphmet_output_0_weight_fixed[EMB_DIM / 2][EMB_DIM];
WT_TYPE graphmet_output_0_bias_fixed[EMB_DIM / 2];
WT_TYPE graphmet_output_2_weight_fixed[1][EMB_DIM / 2];
WT_TYPE graphmet_output_2_bias_fixed[1];

// Function to print all weights of type WT_TYPE
void print_weights_debug()
{
    // Helper lambda to print 2D arrays
    auto print_2d_array = [](const auto& array, int rows, int cols, const std::string& name)
    {
        std::cout << "[DEBUG] " << name << ":\n";
    ...
    // Helper lambda to print 1D arrays
    auto print_1d_array = [](const auto& array, int size, const std::string& name)
    {
        std::cout << "[DEBUG] " << name << ":\n";
    ...
    // Print node embedding weights
    print_2d_array(graphmet_embed_charge_weight_fixed, 3, EMB_DIM / 4, "graphmet_embed_charge_weight_fixed");
    print_2d_array(graphmet_embed_pdgid_weight_fixed, 7, EMB_DIM / 4, "graphmet_embed_pdgid_weight_fixed");
    print_2d_array(graphmet_embed_continuous_0_weight_fixed, EMB_DIM / 2, NUM_CONT_FEAT, "graphmet_embed_continuous_0_weight_fixed");
    print_1d_array(graphmet_embed_continuous_0_bias_fixed, EMB_DIM / 2, "graphmet_embed_continuous_0_bias_fixed");
    print_2d_array(graphmet_embed_categorical_0_weight_fixed, EMB_DIM / 2, EMB_DIM / 2, "graphmet_embed_categorical_0_weight_fixed");
    print_1d_array(graphmet_embed_categorical_0_bias_fixed, EMB_DIM / 2, "graphmet_embed_categorical_0_bias_fixed");
    print_2d_array(graphmet_encode_all_weight_fixed, EMB_DIM, EMB_DIM, "graphmet_encode_all_weight_fixed");
    print_1d_array(graphmet_encode_all_bias_fixed, EMB_DIM, "graphmet_encode_all_bias_fixed");
    print_1d_array(graphmet_bn_all_weight_fixed, EMB_DIM, "graphmet_bn_all_weight_fixed");
    print_1d_array(graphmet_bn_all_bias_fixed, EMB_DIM, "graphmet_bn_all_bias_fixed");
    print_1d_array(graphmet_bn_all_running_mean_fixed, EMB_DIM, "graphmet_bn_all_running_mean_fixed");
    print_1d_array(graphmet_bn_all_running_var_fixed, EMB_DIM, "graphmet_bn_all_running_var_fixed");

    // Print EdgeConv weights
    for (int layer = 0; layer < NUM_LAYERS; ++layer)
    {
        print_2d_array(graphmet_edge_conv_linear_weight_fixed[layer], EMB_DIM, 2 * EMB_DIM, "graphmet_edge_conv_linear_weight_fixed[" + std::to_string(layer) + "]");
        print_1d_array(graphmet_edge_conv_linear_bias_fixed[layer], EMB_DIM, "graphmet_edge_conv_linear_bias_fixed[" + std::to_string(layer) + "]");
        print_1d_array(graphmet_edge_conv_batchnorm_weight_fixed[layer], EMB_DIM, "graphmet_edge_conv_batchnorm_weight_fixed[" + std::to_string(layer) + "]");
        print_1d_array(graphmet_edge_conv_batchnorm_bias_fixed[layer], EMB_DIM, "graphmet_edge_conv_batchnorm_bias_fixed[" + std::to_string(layer) + "]");
        print_1d_array(graphmet_edge_conv_batchnorm_mean_fixed[layer], EMB_DIM, "graphmet_edge_conv_batchnorm_mean_fixed[" + std::to_string(layer) + "]");
        print_1d_array(graphmet_edge_conv_batchnorm_variance_fixed[layer], EMB_DIM, "graphmet_edge_conv_batchnorm_variance_fixed[" + std::to_string(layer) + "]");
    }

    // Print final forward layer weights
    print_2d_array(graphmet_output_0_weight_fixed, EMB_DIM / 2, EMB_DIM, "graphmet_output_0_weight_fixed");
    print_1d_array(graphmet_output_0_bias_fixed, EMB_DIM / 2, "graphmet_output_0_bias_fixed");
    print_2d_array(graphmet_output_2_weight_fixed, 1, EMB_DIM / 2, "graphmet_output_2_weight_fixed");
    print_1d_array(graphmet_output_2_bias_fixed, 1, "graphmet_output_2_bias_fixed");
}

/**
 *
 * Function Implementation for load_weights
 *
...
// Function to Convert float Arrays to WT_TYPE std::vectors
void initialize_node_embedding_parameters() 
{
    ...
    for (int i = 0; i < 3; ++i) 
    {
        ...
        {
            graphmet_embed_charge_weight_fixed[i][j] = static_cast<WT_TYPE>(graphmet_embed_charge_weight_float[i][j]);
        }
    }
    ...
    for (int i = 0; i < 7; ++i) 
    {
        for (int j = 0; j < EMB_DIM / 4; ++j) 
        {
            graphmet_embed_pdgid_weight_fixed[i][j] = static_cast<WT_TYPE>(graphmet_embed_pdgid_weight_float[i][j]);
        }
    }
    ...
    for (int i = 0; i < EMB_DIM / 2; ++i) 
    {
        ...
        {
            graphmet_embed_continuous_0_weight_fixed[i][j] = static_cast<WT_TYPE>(graphmet_embed_continuous_0_weight_float[i][j]);
        }
    }
    ...
    for (int i = 0; i < EMB_DIM / 2; ++i) 
    {
        ...
        {
            graphmet_embed_categorical_0_weight_fixed[i][j] = static_cast<WT_TYPE>(graphmet_embed_categorical_0_weight_float[i][j]);
        }
    }
    ...
    for (int i = 0; i < EMB_DIM; ++i) 
    {
        ...
        {
            graphmet_encode_all_weight_fixed[i][j] = static_cast<WT_TYPE>(graphmet_encode_all_weight_float[i][j]);
        }
    }
    ...
    for (int i = 0; i < EMB_DIM / 2; ++i)
    {
        for (int j = 0; j < EMB_DIM; ++j)
        {
            graphmet_output_0_weight_fixed[i][j] = static_cast<WT_TYPE>(graphmet_output_0_weight_float[i][j]);
        }
    }
    ...
    for (int i = 0; i < EMB_DIM / 2; ++i)
    {
        graphmet_output_2_weight_fixed[0][i] = static_cast<WT_TYPE>(graphmet_output_2_weight_float[0][i]);
    }
...
void initialize_edge_conv_parameters()
{
    ...
    for (int layer = 0; layer < NUM_LAYERS; ++layer)
    {
        ...
        for (int i = 0; i < EMB_DIM; ++i)
        {
            ...
            {
                graphmet_edge_conv_linear_weight_fixed[layer][i][j] = static_cast<WT_TYPE>(weights[i][j]);
            }
        }
        ...
        for (int i = 0; i < EMB_DIM; ++i)
        {
            graphmet_edge_conv_linear_bias_fixed[layer][i] = static_cast<WT_TYPE>(biases[i]);
        }

        // Convert and store batch normalization parameters
        for (int i = 0; i < EMB_DIM; ++i)
        {
            graphmet_edge_conv_batchnorm_weight_fixed[layer][i] = static_cast<WT_TYPE>(bn_weights[i]);
            graphmet_edge_conv_batchnorm_bias_fixed[layer][i] = static_cast<WT_TYPE>(bn_biases[i]);
            graphmet_edge_conv_batchnorm_mean_fixed[layer][i] = static_cast<WT_TYPE>(bn_means[i]);
            graphmet_edge_conv_batchnorm_variance_fixed[layer][i] = static_cast<WT_TYPE>(bn_vars[i]);
        }
    }
}
...
) 
{
    ...
    for (int i = 0; i < num_nodes; ++i)
    {
        for (int j = i + 1; j < num_nodes; ++j)
        {
            ...
            float dist = euclidean(node_feature[i], node_feature[j]);
            ...
            if (dist <= radius)
            {
                ...
                {
                    std::cout << "Maximum number of edges reached.\n";
                    return; // Exit early if max_edges is reached
                }
            }
        }
    }
