...
// @updated (Tu Pham): Fix activation for output
// @date: 11/16/2024
//===========================================================================
...
void GraphMetNetwork::apply_sigmoid(float arr[], int length) {
    for (int i = 0; i < length; ++i)
    {
        arr[i] = 1.0 / (1.0 + std::exp(-arr[i]));
    }
}
...
void GraphMetNetwork::GraphMetNetworkLayers(float x_cont[MAX_NODES][CONT_DIM], int x_cat[MAX_NODES][CAT_DIM], int batch[MAX_NODES], int num_nodes)
{
    ...
    */
    ...
    apply_sigmoid(output, this->_num_nodes);
