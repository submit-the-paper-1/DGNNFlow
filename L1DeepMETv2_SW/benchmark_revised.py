    ...
    *,
    ...
    with torch.inference_mode():
        while warm_left > 0:
            ...
            x_cont_cpu_b = x_cpu_b[:, :6].clone()
            x_cat_cpu_b = x_cpu_b[:, 6:8].to(dtype=torch.long)
            etaphi_cpu = torch.stack([x_cpu_b[:, 3], x_cpu_b[:, 4]], dim=1)
            sizes_t_cpu = torch.tensor(sizes, dtype=torch.long)
            batch_cpu = torch.repeat_interleave(torch.arange(take, dtype=torch.long), sizes_t_cpu)
            edge_index_cpu = radius_graph(etaphi_cpu, r=deltaR, batch=batch_cpu, loop=False, max_num_neighbors=max_neighbors)
            ...
            batch = None
            if device.type == "cuda":
                x_cont = x_cont_cpu_b.to(device)
                x_cat = x_cat_cpu_b.to(device)
    ...
    with torch.inference_mode():
        while meas_left > 0:
            ...
            # Setup
            x_cont_cpu_b = x_cpu_b[:, :6].clone()  # avoid in-place mutation issues
            x_cat_cpu_b = x_cpu_b[:, 6:8].to(dtype=torch.long)
            etaphi_cpu = torch.stack([x_cpu_b[:, 3], x_cpu_b[:, 4]], dim=1)
            sizes_t_cpu = torch.tensor(sizes, dtype=torch.long)
            batch_cpu = torch.repeat_interleave(torch.arange(take, dtype=torch.long), sizes_t)
            assert x_cont_cpu_b is not None and x_cat_cpu_b is not None and etaphi_cpu is not None and batch_cpu is not None

            edge_index_cpu = radius_graph(etaphi_cpu, r=deltaR, batch=batch_cpu, loop=False, max_num_neighbors=max_neighbors)
            assert edge_index_cpu is not None
            E_total = int(edge_index_cpu.size(1))
            ...
            if device.type == "cuda":
                def _h2d():
                    nonlocal x_cont, x_cat, edge_index, batch
                    x_cont = x_cont_cpu_b.to(device)
                    x_cat = x_cat_cpu_b.to(device)
                    edge_index = edge_index_cpu.to(device)
                    batch = batch_cpu.to(device)
                ...
                assert x_cont is not None and x_cat is not None and edge_index is not None and batch is not None
            else:
                x_cont = x_cont_cpu_b
                x_cat = x_cat_cpu_b
                edge_index = edge_index_cpu
